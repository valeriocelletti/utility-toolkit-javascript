...
define(["./Executor"],function(Executor) {
  ...
  /**
   * This constructor simply calls the {@link initTouches} method. This abstract class is supposed 
   ...
   * <br/>For this class to properly work the {@link clean} method that releases the resources needs to be implemented.
   * @constructor
   * 
   * @exports Dismissable
  ...
  Dismissable.prototype = {
       ...
       * a timeout when all of the uses ( i.e. calls to {@link touch} ) were dismissed (using {@link dismiss}).
       * @abstract
       */
      clean: function() {}, 
       ...
       * If not called {@link touch} and {@link dismiss} calls will fail. Ideally this method
       * should be called in the constructor.
       * @protected
       *
       * @param {Number} timeout the time in ms to wait after all the uses were dismissed before actually clean the resources.
       * If a new call to {@link touch} is performed before the timeout is expired the clean is postponed until the 
       ...
       * This method should only be called once per each previous {@link touch} call or the resource may be 
       ...
       * touch call a {@link dismiss} call should be called at some point or the resources will 
       * never be released.
       * 
       * @see dismiss
       */
